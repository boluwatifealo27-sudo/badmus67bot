...
def scrape_black_market_ngn():
    """
    Best-effort scraper — no official free API exists for the parallel market rate.
    Fragile by nature; will break if the source site changes its HTML.
    """
    try:
...
async def convert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not resp or to not in resp["rates"]:
        await update.message.reply_text("Couldn't find that currency pair.")
        return
